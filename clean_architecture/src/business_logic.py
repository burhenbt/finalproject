...
#WORK
	def create_new_item(self, inventory_id : int, name: str, count: int): # name used in "INSERT_INV" constant to create inventories
		"""Gets item ID, name, and count for new item."""
		try:
			self._persistence_wrapper.create_item(inventory_id, name, count) #connects to persistence wrapper "create_item"
		except Exception as e:
			print(f'Exception in Business Logic. {e}')